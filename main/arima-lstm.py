...
class ARIMALSTM(nn.Module):
    def __init__(self, input_size, hidden_size, n_layers, dropout, output_size, arima_order=(1, 0, 0)):
        super(ARIMALSTM, self).__init__()
        self.lstm = LSTM(input_size, hidden_size, n_layers)
        self.dropout = nn.Dropout(dropout)
        self.fc_lstm = nn.Linear(hidden_size, output_size)
        self.arima_order = arima_order
        if arima_order[0] > 0:
            self.fc_ar = nn.Linear(input_size * arima_order[0], output_size)
        else:
            self.fc_ar = None
    ...
    def forward(self, x, state=None):
        ...
        lstm_pred = self.fc_lstm(lstm_out)
        lstm_pred_last = lstm_pred[:, -1, :]

        # ARIMA (AR) 分支
        if self.arima_order[0] > 0 and self.fc_ar is not None:
            p = self.arima_order[0]
            x_trans = x.transpose(0, 1)
            ar_input = x_trans[:, -p:, :].contiguous().view(x_trans.size(0), -1)
            ar_pred = self.fc_ar(ar_input)
        else:
            ar_pred = 0

        combined_pred = lstm_pred_last + ar_pred
        return combined_pred, state
